...
*** Variables ***
${path}    C:\\Kouluhommat\\Liiketoimintaprosessien automatisointi\\Ohjelmistorobotiikka\\Ohjelmistorobotiikka\\
...
Read CSV files to lists and add data to database
    Make Connection    ${dbname}
    ${outputHeader}=    Get File    ${path}ResultFiles\\InvoiceHeaderData.csv
    ${outputRows}=    Get File    ${path}ResultFiles\\InvoiceRowData.csv
    Log    ${outputHeader}
    Log    ${outputRows}
